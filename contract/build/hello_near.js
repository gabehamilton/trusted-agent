...
let FlightPurchase = (_dec = NearBindgen({}), _dec2 = view(), _dec3 = call({}), _dec(_class = (_class2 = class FlightPurchase {
  ...
  // }
}, (_applyDecoratedDescriptor(_class2.prototype, "get_flights", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "get_flights"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "purchase_flight", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "purchase_flight"), _class2.prototype)), _class2)) || _class);
function purchase_flight() {
  const _state = FlightPurchase._getState();
  if (!_state && FlightPurchase._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = FlightPurchase._create();
  if (_state) {
    FlightPurchase._reconstruct(_contract, _state);
  }
  const _args = FlightPurchase._getArgs();
  const _result = _contract.purchase_flight(_args);
  FlightPurchase._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(FlightPurchase._serialize(_result, true));
}
function get_flights() {
  ...
  const _args = FlightPurchase._getArgs();
  const _result = _contract.get_flights(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(FlightPurchase._serialize(_result, true));
}

export { get_flights, purchase_flight };
